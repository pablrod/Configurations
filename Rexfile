...
task 'InstallFedoraPackages', sub {
    update_package_db;
...
desc 'Install packages from other shources than distro repositories';
task 'InstallOtherPackages', sub {

    # NVM
    if ( is_dir("/home/$user/.nvm") ) {
        Rex::Logger::info("NVM already installed");

        # TODO Update nvm
    } else {
        sudo( command => 'curl -o- https://raw.githubusercontent.com/creationix/nvm/v0.32.1/install.sh | bash',
              user    => $user );
    }
